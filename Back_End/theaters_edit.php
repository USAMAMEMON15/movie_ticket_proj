...
$sql ="select * from theaters where id =".$_GET['id'];
				...
				<div class="container-xl">
					<div class="row centre_o1 text-center">
						<div class="col-md-12">
							...
							<h1 class="text-white font_50">Theater-<span id="nav">Edit</span> </h1>
							<h5 class="mb-0 mt-3 fw-normal col_oran"><a class="text-light" href="index.php">Dash-Board</a> <span class="mx-2 text-light">/</span><a href="theaters_show.php" id="nav">Theater Show</a></h5>
						</div>
					</div>
				</div>
			</section>
        ...
        <div class="col-lg-8 m-auto">
            <div class="card  rounded-3 my-5 bg-dark  text-light">
                            <div class="card-header">
                                <h4 class="card-title">Theater Edit</h4>
                            </div>
                            <div class="card-body">
...
<form action="" method="POST">
              <div class="form-group text-dark">
               ...
               <input type="text" class="form-control bg-dark text-light  rounded-3" name="name" required placeholder="name">
             </div>
             <div class="form-group text-dark">
              <label>City</label>
              <input type="text" class="form-control bg-dark text-light  rounded-3 " name="city" required placeholder="City">
             </div>
            <div class="form-group text-dark">
              <label>Address</label>
            <textarea class="form-control bg-dark text-light  rounded-3" name="address" rows="3" placeholder="Adress"></textarea>
           </div>
          <button type="submit" name="update" class="btn my-2">Edit Theater</button>
        </form>
                                </div>
                            </div>
                        </div>
...
if (isset($_POST['update'])){
    ...
    $city = $_POST ['city'];
    $address = $_POST ['address'];
    $sql = "update theaters set name = '$name' ,city = '$city' ,address = '$address'  where id = $id";
    $result = $conn->query($sql);
 ...
 if($result == true){
        ...
        <script>
        Swal.fire({
            title: 'Theater Edit Successfully!',
            icon: 'success',
            showConfirmButton: false,
            timer: 2000,
            didClose: () => {
