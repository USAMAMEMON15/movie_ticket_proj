				...
				<div class="container-xl">
					<div class="row centre_o1 text-center">
						<div class="col-md-12">
							...
							<h1 class="text-white font_50">Comment-<span id="nav">Edit</span> </h1>
							<h5 class="mb-0 mt-3 fw-normal col_oran"><a class="text-light" href="index.php">Dash-Board</a> <span class="mx-2 text-light">/</span><a href="comment_show.php" id="nav">Comment Show</a></h5>
						</div>
					</div>
				</div>
			</section>
...
<form action="" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                <label> Name</label>
                                                <input type="text" name="name" class="form-control bg-dark text-light  rounded-3" placeholder="Name">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>Email</label>
                                                <input type="email" name="email" class="form-control bg-dark text-light  rounded-3" placeholder="Email">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>Phone</label>
                                                <input type="number" name="phone" class="form-control bg-dark text-light  rounded-3" placeholder="Phone">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>City</label>
                                                <input type="text" name="city" class="form-control bg-dark text-light  rounded-3" placeholder="City">
                                            </div>
											<div class="form-group col-md-12">
                                                <label>Comment</label>
                                               	<textarea placeholder="Write a Comment" name="comment" class="form-control form_text bg-dark text-light  rounded-3"></textarea>
                                            </div>
                                        </div>
                                        <button type="submit" name="update" class="btn">Send a Message</button>
                                    </form>
                                </div>
                            </div>
                        </div>
...
if(isset($_POST['update'])){
    ...
    $sql = "update contact set name = '$name',email = '$email',phone= ' $phone',city= ' $city' ,comment= ' $comment' where id  = $id";
    $result = $conn->query($sql);
   ...
   if($result == true){
        ...
        Swal.fire({
            ...
            timer: 2000,
            didClose: () => {
                window.location.href = 'comment_show.php';
            }
        });
        </script>";
    }
