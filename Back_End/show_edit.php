				...
				<div class="container-xl">
					<div class="row centre_o1 text-center">
						<div class="col-md-12">
							...
							<h1 class="text-white font_50">Show-<span id="nav">Edit</span> </h1>
							<h5 class="mb-0 mt-3 fw-normal col_oran"><a class="text-light" href="index.php">Dash-Board</a> <span class="mx-2 text-light">/</span><a href="show_shows.php" id="nav">Show Shows</a></h5>
						</div>
					</div>
				</div>
			</section>
        ...
        <div class="col-lg-8 m-auto">
            <div class="card  rounded-3 my-5 bg-dark  text-light">
                            <div class="card-header">
                                <h4 class="card-title">Show Edit</h4>
                            </div>
                            <div class="card-body">
                                <div class="basic-form">
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                <label>Movies</label>
                                                <select id="inputState" class="form-control bg-dark text-light  rounded-3" name="m_r_id">
                                                   <?php
                                        ...
                                        <div class="form-row">
                                            <div class="form-group col-md-12">
                                                <label>Screens</label>
                                                <select id="inputState" class="form-control bg-dark text-light  rounded-3" name="s_r_id">
                                                   <?php
                                                   $sql = "select * from screens";
                                                   $result = $conn->query($sql);
                                                   while($row =mysqli_fetch_assoc($result)){
                                                    ?>
                                                    <option value="<?php echo $row['id']?>"><?php echo $row['screen_name']?></option> 
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-12">
                                                ...
                                                <input type="datetime-local" class="form-control bg-dark text-light  rounded-3" name ="show_time" placeholder="username">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label>Ticket Price</label>
                                                <input type="number" name ="ticket_price" class="form-control bg-dark text-light  rounded-3" placeholder="Price">
                                            </div>
                                        
                                        </div>
                                     
                                        <button type="submit" name ="update" class="btn my-2 mx-0">Show Edit</button>
                                    </form>
...
if(isset($_POST['update'])){
    $id = $_GET['id'];
    $movie_id  = $_POST['m_r_id'];
    $screen_id  = $_POST['s_r_id'];
    $show_time = $_POST['show_time'];
    $ticket_price = $_POST['ticket_price'];
    $sql ="update shows set movie_id = '$movie_id',screen_id = '$screen_id',show_time = '$show_time',ticket_price = '$ticket_price' where id = $id";
    $result = $conn->query($sql);

    if($result == true){
        echo "<script> 
        alert ('Shows Has Been Edit Succesfuly');
